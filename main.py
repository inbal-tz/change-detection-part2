...
IMAGE_HEIGHT = 128
OUTPUT_DIR = r'C:\Users\inbal.tlgip\Desktop\output_images'
DATA_ROOT = r'C:\Users\inbal.tlgip\Desktop\ChangeDetectionDataset\Real\subset'
BATCH_SIZE = 16
NUM_WORKERS = 0
NUM_EPOCHS = 150
ENCODER_ONLY = False
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        for step, (images, images1, labels, filename) in enumerate(loader_train):  # ChangedByUs
            ...
            if steps_loss > 0 and step % steps_loss == 0:
                ...
                print('loss: {average:', average, '} (epoch: {', epoch, '}, step: {', step, '})',
                      "// Avg time/img: %.4f s" % (sum(time_train) / len(time_train) / BATCH_SIZE))
...
if __name__ == '__main__':
    ...
    model.load_state_dict(
        torch.load(r'C:\Users\inbal.tlgip\modelsave_cropped.pt'))
    model.to(device)
    model.eval()
    # from torchsummary import summary
    # summary(model, (2,3,64,64))
    # print(model)
    co_transform_val = MyCoTransform(ENCODER_ONLY, augment=False,
                                     height=IMAGE_HEIGHT)  # askAlex why we dont augment in val?
    # load test data
    dataset_test = idd_lite(DATA_ROOT, co_transform_val, 'test')
    loader_test = DataLoader(dataset_test, num_workers=NUM_WORKERS, batch_size=1, shuffle=True)
    # calc_TPFPTNFN_on_test_set(model, loader_test)
    ...
    for step, (images, images1, labels, filename) in enumerate(loader_test):
        ...
        if filename[0]=='00084_3':
            for i in range(16):
                cv2.imwrite(r"C:\Users\inbal.tlgip\Desktop\change-detection-part2-master\best heatmaps\16 channels/"+str(i)+".tiff", (GAP[0, i, :, :].squeeze().cpu().detach().numpy()).astype('uint8'))
            import shutil
            shutil.copy(os.path.join(r'C:\Users\inbal.tlgip\Desktop\ChangeDetectionDataset\Real\subset\labels\test_cropped', filename[0] + '.jpg'), r"C:\Users\inbal.tlgip\Desktop\output_images/label.tiff")
            shutil.copy(os.path.join(r'C:\Users\inbal.tlgip\Desktop\ChangeDetectionDataset\Real\subset\images\test\A', filename[0] + '.jpg'), r"C:\Users\inbal.tlgip\Desktop\output_images/A.tiff")
            shutil.copy(os.path.join(r'C:\Users\inbal.tlgip\Desktop\ChangeDetectionDataset\Real\subset\images\test\B', filename[0] + '.jpg'), r"C:\Users\inbal.tlgip\Desktop\output_images/B.tiff")
            x=1
        weights = model.decoder.finalLayer.weight
        change_map = GAP[0, 0, :, :] * weights[1][0]
        for i in range(1, 16):
            change_map += GAP[0, i, :, :] * weights[1][i]
        cv2.imwrite('gap/' + filename[0] + 'BW.png', change_map.squeeze().cpu().detach().numpy())
        fig, ax = plt.subplots()
        plt.imsave('gap/' + filename[0] + 'JET.png', change_map.detach().cpu(), cmap=plt.cm.jet)
        plt.close()
        label = Image.open(r'C:\Users\inbal.tlgip\Desktop\ChangeDetectionDataset\Real\subset\labels\test_cropped/'+filename[0]+'.jpg')
        label = label.convert('1')
        label = ToTensor()(label)
        change_map = softmax(change_map)
        iou_avg_local, recall, precision = calc_iou((((change_map > 0.2) * 255).squeeze().cpu().numpy()).astype('uint8'), label[0, :, :].squeeze().cpu().numpy().astype('uint8'), 0.5)
